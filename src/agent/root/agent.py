...
root_agent = Agent(
    ...
    instruction=(
        ...
        "3. create_mock_map_points を1回呼び、地域名と緯度・経度を渡して地図用座標を返す。\n"
        ...
        "2. create_mock_map_points に selected_route_id（'A', 'B', 'C'など）を渡して1回呼び、そのルートに特化した地図地点を返す。\n"
        "3. 最終回答で、選択されたルートの詳しい時系列タイムライン、地質的な見どころ、移動時の注意点（足元・靴など）を丁寧に解説し、"
        "さらに「滞在時間の短縮やスポットの入れ替えも可能です」と次の対話に繋げる。\n"
        "4. モック情報であることを簡潔に付記する。"
    ),
    sub_agents=[
